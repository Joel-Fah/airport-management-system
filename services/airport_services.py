...
message = "Welcome to the airport management board "
...
def update_airport(airport_data):
    ...
    # Ask for new details
    new_airport_name = input("Enter new airport code (leave blank to keep current) >>> ").strip()

    if new_airport_name == '':
        print("Airport name maintained")

    new_airport_location = input("Enter new location (leave blank to keep current) >>> ").strip()

    if new_airport_location == '':
        print("Airport location maintained")

    new_airport_code = input("Enter new airport code (leave blank to keep current) >>> ").strip()

    if new_airport_code == '':
        print("Airport code maintained")

        new_data = {
            "name": new_airport_name if new_airport_name else airport_data["aiport_name"],
            "location": new_airport_location if new_airport_location else airport_data["aiport_location"],
            "code": new_airport_code if new_airport_code else airport_data["aiport_code"],
        }
        clear_screen()
